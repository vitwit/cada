...
do
    if [ $a == 1 ]
    then
        $DAEMON genesis --home $DAEMON_HOME-$a add-genesis-account validator$a 1000000000000$DENOM  --keyring-backend test
        echo "done $DAEMON_HOME-$a genesis creation "
        continue
    fi
    $DAEMON genesis --home $DAEMON_HOME-$a add-genesis-account validator$a 1000000000000$DENOM  --keyring-backend test
    $DAEMON genesis --home $DAEMON_HOME-1 add-genesis-account $($DAEMON keys show validator$a -a --home $DAEMON_HOME-$a --keyring-backend test) 1000000000000$DENOM
done
...
then
    ...
    do
        ...
        $DAEMON genesis --home $DAEMON_HOME-1 add-genesis-account $($DAEMON keys show account$a -a --home $DAEMON_HOME-1 --keyring-backend test) 1000000000000$DENOM
    done
fi
...
for (( a=1; a<=$NODES; a++ ))
do
    $DAEMON genesis gentx validator$a 90000000000$DENOM --chain-id $CHAINID  --keyring-backend test --home $DAEMON_HOME-$a
done
...
$DAEMON genesis collect-gentxs --home $DAEMON_HOME-1
...
do
    ...
    echo ""/^persistent_peers = \"\"/c\persistent_peers = \"$PERSISTENT_PEERS\"" "$DAEMON_HOME-$a/config/config.toml""
    sed -i "/^persistent_peers = \"\"/c\persistent_peers = \"$PERSISTENT_PEERS\"" "$DAEMON_HOME-$a/config/config.toml"
...
do
    ...
    echo "$DAEMON start --home $DAEMON_HOME-$a"

    echo "$DAEMON status --node tcp://localhost:${RPC}"
...
NOTE : change key, daemon, and node rpc accordingly to post availdata in publish.go