...
func TestFullAppSimulation(t *testing.T) {
	...
	db, dir, logger, skip, err := simtestutil.SetupSimulation(config, "leveldb-app-sim", "Simulation",
	...
	app := NewChainApp(logger, db, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewChainApp(log.NewNopLogger(), newDB, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(AppChainID))
	...
	defer func() {
		if r := recover(); r != nil {
			...
			if !strings.Contains(err, "validator set is empty after InitGenesis") &&
				!strings.Contains(err, "invalid cacheMergeIterator") {
				panic(r)
			}
	...
	storeKeysPrefixes := []StoreKeysPrefixes{
		...
		{app.GetKey(cadatypes.StoreKey), newApp.GetKey(cadatypes.StoreKey), [][]byte{}},
	}
